@tool
extends AsepriteImporterTool

const SyncingAnimationPlayer = preload("./SyncingAnimationPlayer.gd")
...
func _on_animation_player_selected(node_path):
	...
	node_path = root.get_path_to(selected_node)
	...
	if not animation_player:
		...
		animation_player.name = "SyncingAnimationPlayer"
...
func configure(root, node_path, animation_tree, animation_player):
	...
	animation_tree.advance_expression_base_node = animation_tree.get_path_to(root)

	var selected_node = root.get_node(node_path)

	animation_player.source_animation_player = animation_player.get_path_to(selected_node)
